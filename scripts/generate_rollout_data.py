#!/usr/bin/env python3
"""
Rollout-only inference: generate per-turn detector outputs for diagnostic analysis.
Run on base Qwen model without any gradient updates.
"""
import json
import os
import sys
...
def load_scenarios(scenarios_file: str = "data/scenarios/train.jsonl") -> List[Dict]:
    """Load scenario IDs from the training file."""
    scenarios = []
...
def setup_model(model_name: str = "Qwen/Qwen2.5-1.5B-Instruct", device: str = None):
...
def generate_action(model, tokenizer, device, obs, state, max_new_tokens=60):
    """Generate an agent action using the model."""
    # Format prompt
...
def run_rollout(model, tokenizer, device, scenario_id: str, env: RoleDriftEnvironment, rollout_idx: int = 0):
    """Run a single episode and capture per-turn detector outputs."""
    ...
    while not done and state.turn_idx < state.scenario.max_turns:
        ...
        # Get raw detector outputs BEFORE step
...
def main():
    # Load scenarios - use first 20 for the diagnostic
    ...
    # Save results
    output_path = Path("data/diagnostics/rollout_detector_outputs.jsonl")
